...
int main()
{
    ...
    dbusClient.queryAsync([], new ApkDataBaseOperations(
            ApkDataBaseOperations.Enum.listAvailablePackages), null, &asyncCallback, null);
    mainLoop.run();

    return 0;
}

extern (C) void asyncCallback(GObject*, GAsyncResult* res, void* userData)
{
    auto packageVariant = DBusClient.queryFinish(res).getChildValue(0).getChildValue(0);
